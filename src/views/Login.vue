<template>
    ...
    <div class="login-box">
        ...
        <el-form :rules="loginFormRules" ref="loginForm" :model="loginForm" label-position="right" label-width="auto" show-message>
            <span class="login-title">{{$store.state.stu.name}}</span>
            <span class="login-title">{{$store.state.stu}}</span>
            ...
            <el-form-item>
                ...
                <el-button type="primary" @click="onRegit(testname)">注册</el-button>
            </el-form-item>
        </el-form>
        <div>{{count}}</div>
...
<script>
    export default {
        ...
        methods: {
            ...
            onRegit(testname){
                this.$store.commit('onRegit',testname);
            },
            increment(){
                this.count++;
                console.log(count);
